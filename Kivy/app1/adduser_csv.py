...
class MainAPP(MDApp):
    def build(self):
        ...
        return Builder.load_file("adduser_csv.kv")    
    ...
    def save_data(self, user_name, user_surname, user_course, user_email, user_telegram):
        ...
        with open("bd_alumnos.csv", "a+",newline="\n") as file_csv:
            keys = ["Name", "Surname", "Course", "Email", "Telegram"]
            writer = csv.DictWriter(file_csv,fieldnames=keys)
            #writer.writeheader()
            for name, surname, course, email, telegram in aln:
                writer.writerow({"Name": name, "Surname":surname, "Course":course, "Email":email, "Telegram":telegram})
